...
class ReleaseInventoryTests(unittest.TestCase):
    def test_release_inventory_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("tools/live_hosts/gemini_cli_harness.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tools/live_hosts/goose_harness.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tools/live_hosts/grok_build_harness.py", {item["path"] for item in inventory_payload["files"]})
            ...
            self.assertIn("tools/release/discover_adapter_packages.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/policy/runtime_receipts.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/policy/adaptive_lifecycle.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/compiler/small_model_packets.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/specification/completion_gate.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/metrics/outcome_index.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/quality/failure_classification.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/reporting/event_feed.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("src/agent_lifecycle/reporting/progress_view.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("skills/issue-to-spec/SKILL.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/guides/issue-to-spec.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/reference/adaptive-lifecycle-policy.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/reference/model-routing.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/reference/small-model-packets.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/reference/quality-cost-learning.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/ru/reference/adaptive-lifecycle-policy.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/ru/reference/small-model-packets.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("docs/ru/reference/quality-cost-learning.md", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/policy/test_runtime_policy_receipts.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/policy/test_adaptive_lifecycle.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/policy/test_quality_floor.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/compiler/test_small_model_packets.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/specification/test_completion_gate.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/metrics/test_outcome_index.py", {item["path"] for item in inventory_payload["files"]})
            self.assertIn("tests/quality/test_failure_classification.py", {item["path"] for item in inventory_payload["files"]})
