...
class CursorAdapterTests(unittest.TestCase):
    def test_cursor_manifest_is_installable_projection(self) -> None:
        manifest = load_json(ADAPTER_ROOT / ".cursor-plugin" / "plugin.json")
    ...
    def test_descriptor_satisfies_shared_offline_baseline(self) -> None:
        ...
        self.assertEqual(descriptor["host"], "cursor")
    ...
    def test_offline_conformance_descriptor_passes_without_live_runtime(self) -> None:
        conformance = load_json(ROOT / "conformance" / "adapters" / "cursor" / "offline-baseline.json")
        self.assertEqual(conformance["adapterId"], "cursor")
        self.assertFalse(conformance["liveRuntimeRequired"])
        self.assertEqual(conformance["expectedResult"], "PASS")

    def test_cursor_adapter_files_are_project_neutral(self) -> None:
        roots = [ADAPTER_ROOT, ROOT / "conformance" / "adapters" / "cursor"]
        for root in roots:
