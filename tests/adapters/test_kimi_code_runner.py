...
RUNNER = ROOT / "adapters/kimi-code/runner.py"


class KimiCodeRunnerTests(unittest.TestCase):
    def test_run_operation_returns_normalized_host_receipt(self) -> None:
        ...
        self.assertEqual(parsed.operation_id, "kimi-runner-test")
    ...
    def test_run_operation_fails_closed_when_usage_is_missing(self) -> None:
        module = _load_runner()
        request = HostOperationRequest(
            operation_id="kimi-runner-missing-usage",
            capability="launch",
            inputs={},
            outputs=[],
            constraints={},
...
def _load_runner():
    spec = importlib.util.spec_from_file_location("kimi_code_adapter_runner", RUNNER)
    if spec is None or spec.loader is None:
        raise AssertionError("failed to load kimi-code runner")
