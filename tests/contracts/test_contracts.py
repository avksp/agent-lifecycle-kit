...
class ContractTests(unittest.TestCase):
    ...
    def test_schema_registry_is_stable_and_closed(self) -> None:
        ...
        self.assertIn("agent-planning-session-state.v1", ids)
        ...
        self.assertIn("agent-multi-run-attention-item.v1", ids)
        self.assertIn("agent-multi-run-overlap.v1", ids)
        self.assertIn("agent-multi-run-attention-view.v1", ids)
        ...
        self.assertIn("agent-small-model-task-result.v1", ids)
        self.assertIn("agent-small-model-output-validation.v1", ids)
        self.assertIn("agent-small-model-packet-compile-result.v1", ids)
        ...
        self.assertIn("agent-task-change-set-evidence.v1", ids)
        ...
        self.assertEqual(get_schema("agent-small-model-task-packet.v1")["properties"]["executionSurface"], {"const": "small-model"})
        self.assertEqual(get_schema("agent-small-model-task-result.v1")["properties"]["productionPromotionClaimed"], {"const": False})
        ...
        self.assertEqual(get_schema("agent-task-outcome-index.v1")["properties"]["telemetryStarted"], {"const": False})
        self.assertEqual(get_schema("agent-task-outcome-index.v1")["properties"]["monetaryFieldsUsed"], {"const": False})
        self.assertEqual(get_schema("agent-quality-cost-signals.v1")["properties"]["advisoryOnly"], {"const": True})
        self.assertEqual(get_schema("agent-quality-cost-signals.v1")["properties"]["autoApply"], {"const": False})
