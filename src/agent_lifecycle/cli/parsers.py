...
def _add_audit_parser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    delta = audit_sub.add_parser("delta")
    delta.add_argument("--manifest", required=True)
    delta.add_argument("--lock", required=True)
    delta.add_argument("--state", required=True)
    delta.add_argument("--task", required=True)
    delta.add_argument("--dependency-report", required=True)
    delta.add_argument("--validation-selection", required=True)
    delta.add_argument("--finding-check-binding", action="append", default=[])
    delta.add_argument("--finding-check-evidence", action="append", default=[])
