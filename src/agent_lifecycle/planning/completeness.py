...
def validate_plan_completeness_profile(profile: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(profiles, dict):
        ...
        if missing:
            blockers.append(_blocker("missing-tier-profiles", "profiles must contain S0, S1 and S2", {"missing": missing}))
        for tier in sorted(SUPPORTED_TIERS.intersection(profiles)):
            ...
            if not isinstance(required, list) or not all(isinstance(item, str) and item for item in required):
                blockers.append(_blocker("invalid-required-checks", "tier profile requiredChecks must be a string list", {"tier": tier}))
...
def _check_single_workstream(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    ...
    if len(workstreams) != 1:
        blockers.append(_blocker("s0-single-workstream-missing", "S0 plans must contain exactly one workstream", {"tier": tier}))
...
def _check_requirements(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    ...
    if not isinstance(requirements, list) or not requirements:
        blockers.append(_blocker("missing-requirements", "plan specification must contain requirements", {"tier": tier}))
...
def _check_evidence_route(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    criteria = _acceptance_criteria(manifest)
    evidence_ids = set()
    for criterion in criteria:
        ids = _strings(criterion.get("evidenceIds"))
        if not ids:
            blockers.append(_blocker("missing-evidence-route", "acceptance criterion must reference evidence ids", {"criterionId": criterion.get("id")}))
    ...
    if missing:
        blockers.append(_blocker("missing-evidence-route", "evidence ids must be routed to workstreams or validation evidence", {"missing": missing}))
...
def _check_write_ownership(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    ...
    for workstream in workstreams:
        ...
        if not writes:
            blockers.append(_blocker("missing-write-ownership", "workstream writes are required", {"workstreamId": workstream.get("id")}))
...
def _check_validation_command(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    ...
    if not isinstance(commands, list) or not any(isinstance(item, str) and item.strip() for item in commands):
        blockers.append(_blocker("missing-validation-command", "plan validation.commands must contain at least one command", {"tier": tier}))
...
def _check_release_impact(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    if not any(key in manifest for key in ("releaseTarget", "releaseImpact", "nonGoals", "finalAuditGates")):
        blockers.append(_blocker("missing-release-impact", "S1 plans must state release impact or non-release boundary", {"tier": tier}))


def _check_dag(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    workstreams = _workstreams(manifest)
    ids = [item.get("id") for item in workstreams if isinstance(item.get("id"), str)]
    duplicates = sorted({item for item in ids if ids.count(item) > 1})
    if duplicates:
        blockers.append(_blocker("duplicate-workstream-id", "workstream ids must be unique", {"duplicates": duplicates}))
    id_set = set(ids)
    for workstream in workstreams:
        depends_on = workstream.get("dependsOn")
        if not isinstance(depends_on, list):
            blockers.append(_blocker("missing-dag", "S2 workstreams must declare dependsOn", {"workstreamId": workstream.get("id")}))
            continue
        unknown = sorted(item for item in _strings(depends_on) if item not in id_set)
        if unknown:
            blockers.append(_blocker("unknown-workstream-dependency", "workstream dependsOn references unknown ids", {"workstreamId": workstream.get("id"), "unknown": unknown}))
...
def _check_budget_policy(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    ...
    if not has_budget:
        blockers.append(_blocker("missing-budget-policy", "S2 plans must declare token/resource budget policy or validation", {"tier": tier}))
...
def _check_context_limits(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    ...
    if not has_limits:
        blockers.append(_blocker("missing-context-limits", "S2 plans must define context limits or tier byte bounds", {"tier": tier}))


def _check_security_release_gates(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    has_security = bool(_strings(manifest.get("forbiddenWrites"))) or any(key in manifest for key in ("securityGates", "sandbox", "runtimePolicy"))
    has_release = "releaseTarget" not in manifest or bool(_strings(manifest.get("finalAuditGates"))) or _has_path(manifest, ("validation", "commands"))
    if not has_security:
        blockers.append(_blocker("missing-security-gate", "S2 plans must declare security or containment gates", {"tier": tier}))
    if not has_release:
        blockers.append(_blocker("missing-release-gate", "release S2 plans must declare release validation gates", {"tier": tier}))
...
def _check_final_audit_gates(manifest: dict[str, Any], tier: str, blockers: list[dict[str, Any]]) -> None:
    if not _strings(manifest.get("finalAuditGates")):
        blockers.append(_blocker("s2-final-audit-gate-missing", "S2 plans must declare final audit gates", {"tier": tier}))
...
def _has_cycle(workstreams: list[dict[str, Any]]) -> bool:
    graph = {item.get("id"): _strings(item.get("dependsOn")) for item in workstreams if isinstance(item.get("id"), str)}
