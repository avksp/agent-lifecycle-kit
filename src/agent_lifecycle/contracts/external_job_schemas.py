...
_PARENT_ID = {"type": ["string", "null"], "maxLength": 256}
_PARENT_ATTEMPT = {"type": ["integer", "null"], "minimum": 1, "maximum": 1000}
_LIMITS = {
    "type": "object",
    "required": [
        "maxWallSeconds",
    ...
    "properties": {
        "maxWallSeconds": {"type": "integer", "minimum": 1, "maximum": MAX_WALL_SECONDS},
        "maxOutputBytes": {"type": "integer", "minimum": 1, "maximum": MAX_OUTPUT_BYTES},
        "maxArtifactBytes": {"type": "integer", "minimum": 1, "maximum": MAX_ARTIFACT_BYTES},
        "maxArtifacts": {"type": "integer", "minimum": 0, "maximum": MAX_ARTIFACTS},
        "maxCostMicros": {"type": "integer", "minimum": 0, "maximum": MAX_COST_MICROS},
        "maxReportedTokens": {"type": "integer", "minimum": 0, "maximum": MAX_REPORTED_TOKENS},
        "cancelGraceSeconds": {"type": "integer", "minimum": 0, "maximum": MAX_CANCEL_GRACE_SECONDS},
    },
}
_USAGE = {
    "type": "object",
    "required": ["wallMilliseconds", "outputBytes", "artifactBytes", "costMicros", "reportedTokens"],
    "properties": {
...
_CHILD_REF = {
    "type": "object",
    "required": ["jobId", "attempt", "requestDigest"],
    "properties": {"jobId": _TEXT, "attempt": {"type": "integer", "minimum": 1}, "requestDigest": _DIGEST},
}

_REQUEST_FIELDS = [
    "schemaVersion", "jobId", "attempt", "parentJobId", "parentAttempt", "adapterId", "operation",
...
_ARTIFACT_FIELDS = [
    "schemaVersion", "artifactId", "mediaType", "bytes", "sha256", "locator", "sensitiveContentStored",
...
EXTERNAL_JOB_SCHEMAS: dict[str, dict[str, Any]] = {
    EXTERNAL_JOB_REQUEST_SCHEMA: open_object_schema(
        ...
        properties={
            ...
            "parentJobId": _PARENT_ID,
            "parentAttempt": _PARENT_ATTEMPT,
            "adapterId": _TEXT,
            "operation": _TEXT,
            "executionKind": {"enum": list(EXECUTION_KINDS)},
    ...
    EXTERNAL_JOB_STATUS_SCHEMA: open_object_schema(
        ...
        properties={
            ...
            "observedAt": _TEXT,
            "startedAt": {"type": ["string", "null"], "maxLength": 128},
            ...
            "postTerminalWriteDetected": {"type": "boolean"},
            "authorityClaimed": {"const": False},
            "productionPromotionClaimed": {"const": False},
    ...
    EXTERNAL_JOB_ARTIFACT_SCHEMA: open_object_schema(
        ...
        properties={
            ...
            "sensitiveContentStored": {"const": False},
            "productionPromotionClaimed": {"const": False},
    ...
    EXTERNAL_JOB_RESULT_SCHEMA: open_object_schema(
        ...
        properties={
            ...
            "requestDigest": _DIGEST,
            "statusDigest": _DIGEST,
            "state": {"enum": sorted(TERMINAL_JOB_STATES)},
            "verdict": {"enum": list(JOB_VERDICTS)},
    ...
    EXTERNAL_JOB_VALIDATION_SCHEMA: open_object_schema(
        ...
        properties={
            ...
            "subject": {"enum": ["REQUEST", "STATUS", "ARTIFACT", "RESULT"]},
            "subjectState": {"type": ["string", "null"]},
            "blockingEligible": {"type": "boolean"},
            "blockers": _BLOCKERS,
            "productionPromotionClaimed": {"const": False},
...
def build_external_job_request(
    *,
    ...
    _require_parent_pair(body["parentJobId"], body["parentAttempt"])
...
def build_external_job_artifact(
    *, artifact_id: str, media_type: str, bytes_count: int, sha256: str, locator: str
) -> dict[str, Any]:
...
def validate_external_job_artifact(artifact: Any) -> dict[str, Any]:
    blockers: list[dict[str, Any]] = []
    ...
    return _validation("ARTIFACT", None, blockers, blocking_eligible=not blockers)
...
def validate_external_job_status(status: Any, *, request: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    eligible = not blockers and state == "SUCCEEDED"
...
def build_external_job_result(
    *,
    ...
    for artifact in artifacts:
        require_external_job_pass(validate_external_job_artifact(artifact), "artifact")
...
def validate_external_job_result(
    ...
    *,
    ...
    if not isinstance(artifacts, list) or len(artifacts) > MAX_ARTIFACTS:
        ...
        artifacts = []
    for artifact in artifacts:
        if validate_external_job_artifact(artifact)["status"] != "PASS":
    ...
    eligible = not blockers and value.get("blockingEligible") is True
...
def _limits(value: dict[str, int]) -> dict[str, int]:
    ...
    expected = {
        "maxWallSeconds": (1, MAX_WALL_SECONDS),
        "maxOutputBytes": (1, MAX_OUTPUT_BYTES),
        "maxArtifactBytes": (1, MAX_ARTIFACT_BYTES),
        "maxArtifacts": (0, MAX_ARTIFACTS),
        "maxCostMicros": (0, MAX_COST_MICROS),
        "maxReportedTokens": (0, MAX_REPORTED_TOKENS),
        "cancelGraceSeconds": (0, MAX_CANCEL_GRACE_SECONDS),
    }
    if set(value) != set(expected):
        raise LifecycleError("external-job-limits-invalid", "limits fields are invalid")
    return {key: _bounded_int(value[key], low, high, key) for key, (low, high) in expected.items()}


def _usage(value: Any, *, fill_defaults: bool = False) -> dict[str, int]:
    defaults = {"wallMilliseconds": 0, "outputBytes": 0, "artifactBytes": 0, "costMicros": 0, "reportedTokens": 0}
...
def _children(value: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for child in value:
        if not isinstance(child, dict) or set(child) != {"jobId", "attempt", "requestDigest"}:
...
def _check_parent(value: dict[str, Any], blockers: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        _require_parent_pair(parent_id, parent_attempt)
...
def _require_parent_pair(parent_job_id: Any, parent_attempt: Any) -> None:
    if (parent_job_id is None) != (parent_attempt is None):
        raise LifecycleError("external-job-parent-lineage-invalid", "parentJobId and parentAttempt must be paired")
