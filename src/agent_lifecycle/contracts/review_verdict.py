...
BLOCKING_REVIEW_SEVERITIES = frozenset({"BLOCKER", "CRITICAL", "HIGH", "MEDIUM"})
...
def open_blocking_finding_ids(findings: list[dict[str, Any]]) -> list[str]:
    ...
    return [
        ...
        and str(finding.get("status", "")).strip().lower() == "open"
        and str(finding.get("severity", "")).strip().upper() in BLOCKING_REVIEW_SEVERITIES
    ]
...
__all__ = [
    ...
    "REVIEW_VERDICT_VALIDATION_SCHEMA",
