...
from agent_lifecycle.contracts.canonical import canonical_bytes, canonical_digest, load_json_object
...
def validate_domain_language(
    ...
    *,
    ...
    _validate_header(language, blockers)
    term_count, reference_count = _validate_terms(language.get("terms"), blockers)
    _validate_authority_and_source(language, project_root, source_path, blockers)
...
def _validate_header(language: dict[str, Any], blockers: list[dict[str, Any]]) -> None:
    ...
    if not _valid_id(language.get("languageId")):
...
def _validate_terms(value: Any, blockers: list[dict[str, Any]]) -> tuple[int, int]:
    ...
    for index, term in enumerate(terms):
        if not isinstance(term, dict):
            ...
            continue
        reference_count += _validate_term(term, index, term_ids, labels, blockers)
    return term_count, reference_count
...
def _validate_term(
    ...
    elif isinstance(term_id_value, str) and term_id_value in term_ids:
        blockers.append({"code": "domain-language-term-id-duplicate", "termId": term_id_value})
    elif isinstance(term_id_value, str):
    ...
    _validate_labels(term.get("labels"), term_id_value, labels, blockers)
    _validate_aliases(term.get("aliases"), index, term_id, term_id_value, labels, blockers)
    return _validate_references(term.get("references"), index, blockers)


def _validate_labels(
    value: Any, term_id: str | None, labels: dict[tuple[str, str], str], blockers: list[dict[str, Any]]
) -> None:
    if not isinstance(value, dict):
        return
    for locale in DOMAIN_LANGUAGE_LOCALES:
        label = value.get(locale)
...
def _validate_aliases(
    ...
    for alias_index, alias in enumerate(aliases):
        if not isinstance(alias, dict):
            ...
            continue
        value_text = alias.get("value")
        locale = alias.get("locale")
        status = alias.get("status")
        if (
            not isinstance(value_text, str)
            or not value_text.strip()
            or len(value_text.encode("utf-8")) > MAX_DOMAIN_LANGUAGE_TEXT_BYTES
        ):
            blockers.append({"code": "domain-language-alias-value-invalid", "index": index, "aliasIndex": alias_index})
        if locale not in _LOCALES:
            blockers.append({"code": "domain-language-alias-locale-invalid", "index": index, "aliasIndex": alias_index})
        if status not in _ALIAS_STATUSES:
            blockers.append({"code": "domain-language-alias-status-invalid", "index": index, "aliasIndex": alias_index})
        if isinstance(value_text, str) and isinstance(locale, str) and locale in _LOCALES:
            key = (locale, value_text.casefold())
            if key in alias_keys:
                blockers.append({"code": "domain-language-alias-duplicate", "index": index, "value": value_text})
            alias_keys.add(key)
            previous = labels.get(key)
            if previous is not None and previous != term_id:
                blockers.append({"code": "domain-language-alias-ambiguous", "locale": locale, "value": value_text})
...
def _validate_references(value: Any, index: int, blockers: list[dict[str, Any]]) -> int:
    ...
    for reference_index, reference in enumerate(references):
        if not isinstance(reference, dict):
            ...
            continue
        if reference.get("kind") not in _REFERENCE_KINDS:
...
def _validate_authority_and_source(
    language: dict[str, Any], project_root: Path | None, source_path: Path | None, blockers: list[dict[str, Any]]
) -> None:
    expected_authority = {
