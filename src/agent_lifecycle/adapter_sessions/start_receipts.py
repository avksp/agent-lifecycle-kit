...
def _guided_stage(base: dict[str, Any]) -> str:
    ...
    return stages.get(mode, "intake") if isinstance(mode, str) else "intake"
...
def _task_summary(receipt: dict[str, Any]) -> dict[str, Any]:
    planning_value = receipt.get("planningImport")
    ...
    recommendation_value = receipt.get("reviewMeshRecommendation")
    recommendation: dict[str, Any] = recommendation_value if isinstance(recommendation_value, dict) else {}
    return {
...
def _claims_execution(receipt: dict[str, Any]) -> bool:
    return any(
        bool(receipt.get(field)) for field in ("executionStarted", "hostLaunchStarted", "lifecycleCoverageClaimed")
    )
...
def _redact_task_echo(value: Any, *, task_text: str) -> Any:
    ...
    if isinstance(value, dict):
        return {str(key): _redact_task_echo(item, task_text=task_text) for key, item in value.items()}
    return value
...
def _blocker_summaries(value: Any) -> list[dict[str, Any]]:
    ...
    for item in value:
        if isinstance(item, dict):
            summary: dict[str, Any] = {"code": str(item.get("code", "start-delegate-blocked"))}
