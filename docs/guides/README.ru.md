...
Lifecycle Kit превращает эти шаги в явные artifacts и gates, чтобы задачу можно
было довести до проверяемого завершения без потери качества и контроля
ресурсов.

Используйте его, когда нужны:

- проверенное SDD-планирование до старта реализации;
- frozen task packets с ограниченными ownership и validation contracts;
- контролируемое выполнение с context, budget и external-action gates;
- независимый implementation audit перед acceptance;
- host-specific adapter evidence вместо широких неподтверждённых claims.

## Текущий статус

`v0.12.1` — текущий source release. Он включает release-0-5 execution gates,
release-0-6 Codex CLI live adapter evidence, adapter capability manifests,
safe adapter inspection, offline adapter conformance verification и ordered
terminal outcomes для OpenCode, Hermes, Cursor, Gemini CLI, qwen-code и
Kimi Code.
...
checkout. Будущий npm package может ссылаться на тот же adapter, но `v0.12.1`
