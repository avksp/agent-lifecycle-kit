...
Этот документ показывает, как независимо проверить одно исследование, план или
реализацию несколькими моделями ИИ через Review Mesh. Каждая проверка может
использовать отдельную связку адаптера и модели, например Codex, Claude Code и
OpenCode/GLM. ALK готовит задания, импортирует ответы, объединяет выводы и
проверяет кворум. Модели запускает оператор или обёртка хоста; ALK не становится
посредником провайдера. Техническое имя CLI-команды остаётся `review-mesh`.

Примеры не задают фиксированный состав. Можно использовать любые доступные
адаптеры и модели, включая локальные. Review Mesh необязателен и по умолчанию
выключен. Если доступна только одна модель, обычное планирование и аудит одним
рецензентом остаются допустимыми. Несколько сеансов одной модели не доказывают
независимость по модели, когда профиль требует измерения `host` и `model`.

После нескольких завершённых проверок можно использовать [оптимизацию проверок
по подтверждённым данным](reference/audit-optimization.md), чтобы собрать
локальную статистику качества, времени, токенов и ресурсов. Это только
рекомендация: сначала нужны эталонные задачи и явное подтверждение оператора.
...
проверки.
Короткая карта сценариев находится в
[практических сценариях жизненного цикла](lifecycle-cookbook.md).
...
признаки задачи по исправлению ошибки. Профиль расследования может быть рекомендован для
...
- Режим обязателен только для этапов, которые этот план сделал блокирующими.
- Если требуемые независимость или кворум недостижимы, исправьте и заново
  зафиксируйте план вместо подмены идентификаторов или обхода проверки.
