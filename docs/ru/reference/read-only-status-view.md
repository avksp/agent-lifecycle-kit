...
обёрток адаптеров. Он фиксирует идентификатор адаптера, уровень поддержки
прогресса, точку подключения, строки для терминала и отпечаток исходного
представления прогресса или подтверждения наблюдения.
...
Отображение только показывает данные: оно не является источником правды, не
разбирает телеметрию конкретного хоста в ядре, не вычисляет токены
самостоятельно и не запускает модель.
