...
Граница производителя фиксируется как `adapter-owned`.

Автоматическая установка: нет. Ядро ALK не устанавливает hook хоста и не
записывает настройки хоста. Адаптер или оператор может подготовить
`agent-adapter-event-stream-receipt.v1` через обёртку или собственный hook
хоста, но переносимый артефакт должен оставаться нейтральным.
...
статус автоматической установки для встроенных адаптеров.
