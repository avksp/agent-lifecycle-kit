...
Review Mesh — это дополнительный слой контрактов для независимой проверки
одного ограниченного материала несколькими моделями ИИ. Каждый проверяющий
может использовать отдельную связку адаптера и модели. Такой режим полезен,
когда плану нужно больше одного независимого взгляда на исследование, план или
подтверждения реализации, но не входит в базовый жизненный цикл. Техническое
имя команды: `review-mesh`.

Названия продуктов не ограничивают выбор: можно использовать любые доступные
связки адаптеров и моделей. Встроенные шаблоны требуют независимости по `host`
и `model`. Несколько сеансов одной модели могут дать дополнительные советы, но
не выполняют требование независимости по модели.
...
Установка ALK или формулировка задачи не делают Review Mesh обязательным.
Обязательность появляется только тогда, когда проверенный зафиксированный план
требует блокирующий кворум для конкретного этапа. Если другой модели нет,
оставьте режим выключенным или используйте обычную проверку одним рецензентом.
Если независимый кворум уже требуется зафиксированным планом, отсутствие
доступных моделей является блокировкой до исправления и повторной фиксации
плана.

## Ограниченные раунды и исходы находок

План может задать `maxPlanReviewRounds` от 1 до 10. Этот бюджет относится к
проверке плана и synthesis Review Mesh, а не к попыткам задач workflow, которыми
управляет `maxTaskAttempts`. Если бюджет исчерпан при открытой находке
`MEDIUM`, `HIGH`, `CRITICAL` или `BLOCKER`, результатом будет
`REPLAN_REQUIRED`, `SPLIT_REQUIRED`, `OPERATOR_DECISION` или `BLOCKED`, но не
acceptance.

Участие проверяющего засчитывается только для schema-valid
`agent-review-verdict.v1`, связанного с полным blocking-eligible
`agent-external-job-result.v1` успешно завершившейся terminal job.
`NO_FINAL_VERDICT`, успешный exit code процесса или импорт только находок можно
учесть в расходах, но нельзя засчитать как участие или acceptance.

Каждая импортированная находка связывается с одним неизменяемым
`agent-finding-disposition.v1`: `CONFIRMED`, `REJECTED`, `UNAVAILABLE` или
`APPROVAL_REQUIRED`. Категории synthesis и согласие проверяющих не закрывают
находку. Только совпадающий `REJECTED` disposition делает открытое ложное
срабатывание неблокирующим; находка без disposition и другие открытые
Medium-or-higher находки остаются блокерами.

Поле reviewer `reproduction` остаётся недоверенной рекомендацией. Оно может
создать `agent-finding-check-proposal.v1`, но выполнение требует отдельно
одобренного и зафиксированного finding-check binding через существующую
shell-free границу процесса. Текст проверяющего никогда не становится argv.

Для статистических заявлений см. [независимость подтверждений](evidence-independence.md),
для рекомендательного учёта расходов - [эффективность проверки](review-efficiency.md).
