...
В релизе 1.41 добавлена детерминированная рекомендация режима. В релизе 1.42
добавлен полуавтоматический слой вокруг неё: назначения проверяющим, импорт
результатов, объединение выводов и проверка кворума. ALK по-прежнему не запускает адаптеры
проверяющих: оператор или обёртка хоста запускает их отдельно и возвращает
подтверждение для импорта.
...
подтверждения Review Mesh, не становясь посредником для моделей:
