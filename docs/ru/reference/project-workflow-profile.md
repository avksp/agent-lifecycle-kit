...
При управляемом запуске задачи этот порядок записывается в
`agent-execution-strategy.v1`. Артефакт связывает точный отпечаток и путь
эффективного профиля либо каноническое состояние `ABSENT`. Изменённый профиль
не влияет на уже начатую попытку и не может быть незаметно переиспользован в
следующей: создайте новую стратегию через `--strategy-out`. Подтверждение
возможностей влияет только на исполнимость выбранного маршрута и никогда не
выбирает более слабое значение.
