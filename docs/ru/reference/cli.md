...
## Миграция артефактов полномочий на 2.15

Версия 2.15 усиливает проверки до использования артефактов как источника
полномочий. Для корректного JSON с уникальными ключами сохранены канонические
байты, digest и завершающий перевод строки версии 2.14. Исторические manifest,
lock и receipts не переписываются. Исправляйте некорректный ввод в новой ревизии
плана; изменение входов, определяющих полномочия, требует повторного независимого
ревью и заморозки.

- Ключи JSON должны быть уникальны на любой глубине, включая эквивалентные
  escape-записи. Неоднозначный ввод отклоняется с `invalid-json` и
  `JSON input is invalid`; диагностика не раскрывает ключи, значения, пути и payload.
- Чтение и запись артефактов полномочий используют защищённую границу I/O,
  привязанную к корню. Обход каталогов, выход через симлинки, Windows/UNC-алиасы
  и подмена предков приводят к отказу. Отсутствие необходимого примитива платформы
  не превращает структурную проверку в разрешение операции.
- Декларируйте пути полномочий в NFC. Однозначное имя из файловой системы
  сравнивается в NFC с сохранением исходных байтов имени для I/O и evidence.
  На проверенном регистрозависимом корне `Foo.py` и `foo.py` остаются разными
  путями; на регистронезависимом корне NFC и casefold выявляют неоднозначные
  и защищённые алиасы. Неизвестная семантика файловой системы и неоднозначность
  нормализации приводят к отказу. Политика одинакова для `readOnly`,
  `forbiddenWrites`, `leadOwned[].path` и `workstreams[].writes`;
  нормализация не расширяет право записи.
- Токены и пути полномочий отклоняют управляющие символы и символы визуальной
  подмены. Текст, влияющий на решения, сохраняет TAB/LF/CR и языковые соединители
  U+200C/U+200D при той же защите от подмены. Обычный многоязычный текст,
  включая многострочный английский и русский, остаётся допустимым.
  Ошибка `authority-text-control` не раскрывает опасный текст и не исправляет
  поле незаметно для проверки.
- Владение changeset учитывает удалённую и добавленную стороны переименования.
  При копировании учитывается изменённый путь назначения; неизменённый исходник
  не приписывается задаче.
- Компилятор проверяет все пути пакетов и индекса до создания каталогов и файлов.
  Декларированные выходы, выходы по умолчанию и явный `--out-dir` не могут
  пересекать защищённый инвентарь плана или выходить за корень репозитория.
  Безопасные соседние каталоги, непересекающиеся выходы с общим родителем
  и абсолютные пути экспорта внутри репозитория поддерживаются. Явный экспорт
  не переопределяет привязки последующего adoption. Защищённая запись сохраняет
  изоляцию после предварительной проверки; общей гарантии rollback при иных
  ошибках I/O это не добавляет.
- `PASS` при отмене внешней задачи подтверждает публикацию запроса отмены.
  Он не доказывает завершение процесса: терминальное evidence и очистка остаются
  отдельными действиями. Сохранены исходные терминальные no-op, идемпотентность
  и привязки происхождения.

См. [Границы полномочий и артефактов](authority-artifact-boundaries.md):
`operation_root`, запуск задачи и аудит пакета. PASS проверки completeness без
корня подтверждает только автономную структуру. Evidence валидаторов привязано
к проверенному исходному коду и фактическим потребителям защищённого I/O.
Эти изменения не подтверждают квалификацию хоста `ENFORCED` или готовность релиза.
Полные проверки регрессий, безопасности, архитектуры, нейтральности, качества,
документации и публикации обязательны с прежними порогами.
