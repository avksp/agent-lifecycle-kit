...
Каждый ограниченный управляемый процесс также создаёт обезличенную квитанцию
`agent-process-execution-receipt.v1`. В ней указаны монотонная длительность,
доступные счётчики процессора, памяти и числа процессов, признак тайм-аута или
отмены и результат очистки. Объедините квитанции командой
[`metrics execution-report`](cli.md), чтобы проверить расход ресурсов
без сохранения аргументов, окружения или вывода хоста. Отсутствующая или
неудачная очистка блокирует результат и не скрывается повторной попыткой.
