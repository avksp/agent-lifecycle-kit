...
явным запуском процесса через проверенный профиль описаны в разделе [использование ALK
...
`managedLaunch.status: WRAPPER_ONLY`: проверенные профили задают маршрут
