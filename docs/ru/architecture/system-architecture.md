...
Когда независимая проверка или аудит возвращает `REWORK`, то же ядро проверяет
открытые находки и зафиксированный лимит попыток, сохраняет неизменяемые
идентификаторы текущей попытки и переводит задачу в `REMEDIATING`. Следующий
запуск открывает свободный путь новой попытки и не заменяет прошлый результат,
проверку или аудит.
