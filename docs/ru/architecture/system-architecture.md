...
5. **Подтверждения задачи и проверок** показывают выполненную работу,
   запущенные проверки, изменённые файлы, актуальный снимок содержимого по Git,
   расход ресурсов и нерешённые действия.
...
Для результата задачи ALK не считает время создания файла или заявление модели
доказательством актуальности. Он разрешает зафиксированную исходную ревизию,
получает текущий набор файлов Git и вычисляет отпечатки исходного и текущего
состояния файлов задачи. Рабочий цикл и аудит реализации повторяют эту
проверку. Git подтверждает, какие байты проверялись, а тесты и независимая
проверка по-прежнему определяют, соответствует ли их поведение требованиям.
...
## Граница целостности плана в релизе 1.79

Релиз 1.79 добавляет каноническую безопасную проверку передачи пакета плана
другому исполнителю или проверяющему. Слой `contracts` задаёт форму манифеста и
общие правила для буквальных путей полномочий. Слой `planning` проверяет
трассируемость требований, критериев приёмки, подтверждений и итоговых гейтов,
а затем формирует `agent-plan-verification-receipt.v1`. CLI только загружает
явно указанные входы и сериализует результат; он не становится источником
полномочий рабочего цикла.

Пути принятия плана и управляемого запуска используют те же проверки контракта
и полноты до записи состояния. Целостность пакета остаётся ответственностью
слоя заморозки и использует тот же `agent-plan-lock.v2`, поэтому проверка плана
не создаёт второй алгоритм инвентаризации или проверки lock-файла. Для
зафиксированного плана требуется совпадающий lock-файл; черновик можно
проверить без него, но он не даёт полномочий на реализацию.

Фасад проверки не выполняет команды из плана, не вызывает модели, не обращается
к сети и не запускает адаптер. Хост и модель отвечают за исследование и работу
с кодом, а ALK - за структурированные полномочия, переходы состояния,
подтверждения и закрытые решения при передаче работы. См. [проверка
целостности плана](../reference/plan-verification.md).

## Релиз 1.80: необязательный контроль жизненного цикла адаптера

Релиз 1.80 добавляет ограниченную границу контроля на уровне адаптера. Она
необязательна и не зависит от провайдера. Модель по-прежнему предлагает
содержание и использует инструменты хоста; хост или адаптер остаётся
производителем наблюдаемых событий; ALK проверяет договоры событий, решений и
подтверждений и связывает их с существующим планом и состоянием рабочего
цикла.

Граница контроля публикует четыре поля для каждой операции:
`declaredLevel`, `supportedLevel`, `qualifiedLevel` и
`qualificationStatus`. Уровни `GUIDANCE_ONLY`, `OBSERVED` и `ENFORCED` означают
следующее:

- `GUIDANCE_ONLY` - указания из навыка или страницы адаптера. Полномочий на
  блокировку нет.
- `OBSERVED` - активность записывается проверенным производителем на стороне
  хоста. Это может остановить последующую приёмку, но не доказывает, что
  действие было предотвращено.
- `ENFORCED` - хост блокирует действие до запуска, а проверки после действия и
  остановки связывают результат с рабочим циклом. Уровень допустим только для
  точной версии хоста и операции, покрытой положительными и отрицательными
  подтверждениями.

Слои разделены намеренно. Модули договоров задают ограниченные и очищенные
формы запроса, решения, событий и квалификации. Модули host-protocol разрешают
настройку операции и выполняют чистые контрольные решения. Слой рабочего цикла
использует принятые подтверждения после действия при приёмке задачи и
финализации. CLI только сериализует проверки и не запускает модели и не
становится источником полномочий плана. Проекции адаптеров отвечают за
настройку native hook, а настройки хоста и ключи подтверждений находятся вне
области записи модели.

Сейчас все комплектные адаптеры публикуют для операций контроля жизненного
цикла `GUIDANCE_ONLY` и `NO_RECOMMENDATION`, а для управляемого запуска -
`WRAPPER_ONLY`. Это честный результат проверки возможностей: объявления,
промпты, установка плагина, фикстуры событий и успешный положительный сценарий
не повышают уровень адаптера. Отсутствие ALK, тайм-аут, неверный ответ,
отключённый контроль и остановка производителя не являются подтверждением
квалификации. См. [необязательный контроль жизненного цикла адаптера](../adapters/lifecycle-control.md)
и [захват событий адаптера](../reference/adapter-event-capture.md).

## Релиз 1.83: явное восстановление и маршрутизация финального аудита

Релиз 1.83 сохраняет ядро операции единственным писателем состояния и делает
маршруты восстановления явными. `workflow authorize` принимает ограниченное
подтверждение с точной связью запуска, плана и исходной ревизии; `PLAN_ONLY` -
отдельная неисполняемая фаза, которую нельзя повысить таким подтверждением.
Пауза и возобновление внешнего действия несут идентичность объявленного
действия и соответствующее подтверждение.

Блокеры типизированы по области (`run`, `task`, `plan` или `external`) и
содержат закрытый маршрут восстановления. Общий резолвер запуска не может
снять блокер задачи или плана. Решения о разделении и исчерпании бюджета
обновляют состояние задачи и запуска согласованно, а исход финального аудита
применяется одной службой контроллера: `ACCEPTED` продолжает существующие
гейты финализации, `REWORK` архивирует названные попытки в пределах
замороженного бюджета, `CONTRACT_CHANGE` ждёт новый зафиксированный план, а
...
В состоянии workflow v4 результаты ревью локальны для задачи. Канонический
сервис `task-review-apply` проверяет текущий результат, независимое ревью,
lineage и свежесть перед применением `ACCEPTED`, `REWORK`,
`CONTRACT_CHANGE` или `BLOCKED`. Активный сосед не переводится в общий этап
ревью. Для `REWORK` то же ядро проверяет открытые идентификаторы находок и
замороженный бюджет повторов, сохраняет неизменяемые идентичности артефактов
текущей попытки и открывает неиспользованный путь следующей попытки; прежние
результат, ревью и аудит не заменяются. Состояние переходит в `FINAL_AUDIT`
только после принятия всех обязательных задач. Legacy-состояние v3 требует
явной fail-closed миграции в v4.

То же ядро операции применяет исходы финального аудита. Перед изменением
состояния оно проверяет независимого проверяющего, точную связь запуска,
плана и источника и, для `REWORK`, связь каждой находки с названной задачей.
При ошибке проверки состояние и журнал событий остаются неизменными.
Управляемый runner показывает эти действия хосту, но не считает их операциями
модели или редактирования файлов.
