...
## Границы качества и поставки в релизе 1.77

Релиз 1.77 добавляет контроль сопровождения поверх существующей архитектуры
жизненного цикла. Ruff, mypy и coverage работают только при разработке через
ограниченный релизный запускатель; они не становятся зависимостями выполнения
и не получают новых полномочий на запуск. Политика качества связывает старые
находки с отпечатками исходников и не допускает новых или растущих находок.

Корневой CLI остаётся границей сборки приложения. Он переводит ожидаемые
ошибки ввода-вывода, декодирования, глубины JSON и непредвиденные ошибки в
стабильный контракт `agent-lifecycle-error.v1`, а доменные библиотеки сохраняют
свои исключения и поведение прерываний. Поэтому CLI предоставляет безопасную
машиночитаемую границу, не перенося правила жизненного цикла в парсер.

Встроенные профили принадлежат слою ресурсов пакета. Проверенные исходные и
установленные копии сравниваются по отпечатку, а `importlib.resources` находит
их независимо от текущей папки. Явно переданный оператором путь имеет больший
приоритет; файл проекта не может подменить встроенное значение по умолчанию.

Поддерживаемая поверхность API Python задаётся явным списком корневого пакета
и выбранных фасадов. Валидатор проверяет импорт, экспорт, аннотации и
документацию на русском и английском языках. Внутренние модули остаются
деталями реализации, пока не перечислены в этом контракте.

## Границы производительности в релизе 1.78

Релиз 1.78 ускоряет повторяющиеся локальные операции, не меняя полномочия
жизненного цикла. Политика производительности и типизированные пределы
отделены от источников полномочий плана, lock-файла и критериев приёмки:
лучший результат измерения не может принять реализацию или заменить
обязательную проверку безопасности.

Реализация Ed25519 использует итеративную арифметику в расширенных координатах
и проверяется по опубликованным векторам, принятым квитанциям и повреждённым
входам. Сканирование нейтральности использует ограниченные пакетные потоки Git
и дифференциальное сопоставление строк; идентичность дерева читает файлы
потоково с ограничением; обход группы процессов Linux выполняется с отдельной
частотой; корневой CLI загружает группу команд только после её выбора. Каждая
оптимизация сохраняет запуск без оболочки, отказ по умолчанию при нарушении
лимитов, устойчивое чтение, снимки дерева до и после, редакцию секретов и
детерминированные доказательства.

Измерительный контур сохраняет ревизию, окружение, выборку и ограниченные
сводки операций в `work/`. Время и память являются справочными, если только
план явно не сделал их критерием. Единственный временной критерий 1.78 —
чередуемое сравнение медиан Ed25519 с зафиксированной аффинной реализацией;
заявления о постоянном времени и кэширование результатов отсутствуют. См.
[бюджеты производительности и ресурсов](../reference/performance-and-resource-budgets.md).
...
### Артефакты непрерывности проекта

Слой профиля проекта может ссылаться на ограниченный артефакт
`agent-project-principles.v1` по пути и отпечатку. Модуль
`project/principles.py` проверяет его границы и полномочия, а
`project/profile.py` и `project/merge.py` передают в эффективный профиль только
ссылку. Файл принципов является контекстом, а не спецификацией и не источником
полномочий на выполнение.

`planning/deltas.py` сравнивает две явно указанные версии плана. Он сводит
требования, пути записи, приёмку, подтверждения, бюджеты, риски и контрольные
точки к кратким изменениям с отпечатками. Команда `plan delta` не изменяет
манифест или lock-файл; при изменении полномочий она устанавливает
...
| Профиль рабочего процесса проекта и встроенные профили | `project/profile.py`, `project/merge.py`, `project/presets.py`, `project/guidance.py`, `cli/project.py` | `project profile`, `project preset list/inspect/validate/render` и `start`, когда выбран локальный профиль или встроенный маршрут. Встроенный профиль задаёт ограниченные значения по умолчанию, а профиль проекта, CLI и зафиксированный план сохраняют отдельные полномочия. |
...
Для долгосрочного управления проектом маршрут выглядит так:

```mermaid
sequenceDiagram
  participant O as Оператор
  participant C as CLI
  participant P as Профиль проекта
  participant D as Дельта плана
  participant R as Проверяющий

  O->>C: project principles check
  C->>P: validate_project_principles
  P-->>C: проверка с отпечатком
  O->>C: plan delta --before --after
  C->>D: build_plan_delta
  D-->>C: влияние на полномочия
  C-->>R: reviewRequired/newLockRequired
  R->>C: обычное ревью и заморозка плана
```
...
| Встроенный профиль рабочего процесса | `project preset list/inspect/validate/render`, `start --preset` | `cli/project.py`, `project/presets.py`, `project/profile.py`, `project/merge.py`, `contracts/project_profile_preset_schemas.py` | Локальный список, проверка, результат создания профиля или настройки для запуска; модель и внешний процесс не запускаются. |
