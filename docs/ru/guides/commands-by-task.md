...
Если точный следующий переход неизвестен, используйте `workflow continue` в
два шага: сначала вычислите действие без `--apply`, затем повторите те же входы
с `--apply`, вычисленной ревизией и дайджестом действия. Прямые команды
переходов сохраняются. См. [продолжение
workflow](../reference/workflow-continuation.md).
Если несколько детерминированных переходов уже объявлены, пакетный режим может
применить их явный набор за один вызов. Нужны `--until-blocked --apply`, lock,
положительные лимиты переходов/I/O и отдельный выходной receipt; до внешних
полномочий или доказательств ALK останавливается.
