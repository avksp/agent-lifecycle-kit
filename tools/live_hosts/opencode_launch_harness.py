...
def run_preflight(approval_path: Path) -> dict[str, Any]:
    ...
    receipt = None
    if not blockers:
        with tempfile.TemporaryDirectory(prefix="alk-opencode-preflight-") as tmp:
            root = Path(tmp)
            profile_path = root / ".alk/host-launch/opencode.json"
...
def run_planning_preflight(approval_path: Path) -> dict[str, Any]:
    ...
    if not blockers:
        with tempfile.TemporaryDirectory(prefix="alk-opencode-planning-") as tmp:
            root = Path(tmp)
