...
HARNESS_REPORT_SCHEMA = "agent-claude-code-live-harness-report.v1"
...
class ClaudeUsage:
    ...
    def with_context_byte_proxy(self, value: int) -> "ClaudeUsage":
        return replace(
            self,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--diagnostic-dir", default="tasks/release-0-3/evidence/live-host-diagnostics/claude-code")
...
def run_preflight(
    *,
    ...
    version = _run_command([claude_bin, "--version"], checks, "claude-version")
    help_result = _run_command([claude_bin, "--help"], checks, "claude-help")
    print_help = _run_command([claude_bin, "--print", "--help"], checks, "claude-print-help")
    ...
    help_ok = all(result["returncode"] == 0 for result in (version, help_result, print_help))
    return {
        **_base_report("PASS" if not blockers and help_ok else "FAIL", blockers),
        "checks": checks,
        "claudeVersion": _first_line(version["stdout"]),
    ...
    if not blockers:
        ...
        _write_json(
            receipt_path,
            {
                ...
                "status": "PASS",
                "receiptId": "claude-code-live-host-conformance",
                "host": HOST,
                "adapterId": HOST,
    ...
    if not blockers:
        ...
        _write_json(
            receipt_path,
            {
                ...
                "status": "PASS",
                "receiptId": "claude-code-live-calibration",
                "host": HOST,
...
def build_live_operation_record(*, host: str, name: str, invocation_id: str, usage: ClaudeUsage, output_identity: dict[str, Any]) -> dict[str, Any]:
...
def parse_claude_jsonl(text: str, *, wall_seconds: float = 0.0) -> ClaudeUsage:
    ...
    tool_calls = 0
    for event in events:
        session_id = session_id or _find_string(event, {"session_id", "sessionId"})
        usage_values.extend(_find_dicts(event, {"usage"}))
    ...
    cache_tokens = sum(_int_from_any(_first_present(usage, ("cache_creation_input_tokens", "cacheCreationInputTokens", "cache_read_input_tokens", "cacheReadInputTokens"))) for usage in usage_values)
...
def _validate_calibration_inputs(profile: dict[str, Any], targets: dict[str, Any], blockers: list[dict[str, Any]], scenarios: list[str], cohorts: list[str], requested_runs: int, minimum_runs: int) -> None:
    ...
    if HOST not in _strings(profile.get("requiredHosts")):
        blockers.append({"code": "live-calibration-host-unsupported", "message": "claude-code is not in requiredHosts"})
...
def _usage_or_block(result: CommandResult, budget_policy: BudgetPolicy, blockers: list[dict[str, Any]], label: str) -> ClaudeUsage | None:
    if result.returncode != 0:
        blockers.append({"code": "claude-code-live-invocation-failed", "message": f"{label} returned {result.returncode}"})
        return None
    usage = parse_claude_jsonl(result.stdout, wall_seconds=result.wall_seconds)
...
def _calibration_usage_or_block(result: CommandResult, budget_policy: BudgetPolicy, blockers: list[dict[str, Any]], label: str, prompt: str) -> ClaudeUsage | None:
...
def _prompt_for_calibration(scenario: str, cohort: str, run_index: int) -> str:
    return (
        "Agent Lifecycle Kit Claude Code live calibration probe. "
