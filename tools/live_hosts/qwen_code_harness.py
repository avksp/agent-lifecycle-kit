...
HARNESS_REPORT_SCHEMA = "agent-qwen-code-live-harness-report.v1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--qwen-fallback-model")
    ...
    parser.add_argument("--diagnostic-dir", default="tasks/release-0-11/evidence/live-host-diagnostics/qwen-code")
...
def run_preflight(
    *,
    ...
    version = _run_command([qwen_bin, "--version"], checks, "qwen-code-version")
    help_result = _run_command([qwen_bin, "--help"], checks, "qwen-code-help")
    extensions_help = _run_command([qwen_bin, "extensions", "--help"], checks, "qwen-code-extensions-help")
    mcp_help = _run_command([qwen_bin, "mcp", "--help"], checks, "qwen-code-mcp-help")
    ...
    help_ok = all(result["returncode"] == 0 for result in (version, help_result, extensions_help, mcp_help))
    return {
        **_base_report("PASS" if not blockers and help_ok else "FAIL", blockers),
        "checks": checks,
        "qwenCodeVersion": _first_line(version["stdout"]),
    ...
    *,
    ...
    for index, operation_name in enumerate(operations, start=1):
        try:
            ...
            blockers.append({"code": error.code, "message": error.message})
            break
        invocation_id = f"qwen-code-{operation_name}-live-{index:02d}"
        result = command_runner(_operation_command(qwen_bin, worktree, operation_name, qwen_model=qwen_model, qwen_fallback_model=qwen_fallback_model, model_selection=model_selection))
        live_calls_started = True
        transcript = _write_invocation_diagnostic(diagnostic_dir, operation_name, invocation_id, result)
        checks.append({"name": f"qwen-code-live-{operation_name}", "status": "PASS" if result.returncode == 0 else "FAIL", "details": {"returncode": result.returncode, "diagnostic": _display_path(transcript)}})
    ...
    if not blockers:
        ...
        _write_json(
            receipt_path,
            {
                ...
                "status": "PASS",
                "receiptId": "qwen-code-live-host-conformance",
                "host": HOST,
                "adapterId": HOST,
        ...
        for cohort in cohorts:
            for run_index in range(1, requested_runs + 1):
                try:
                    ...
                    blockers.append({"code": error.code, "message": error.message})
                    break
                invocation_id = f"qwen-code-{scenario}-{cohort}-{run_index:02d}"
                prompt = _prompt_for_calibration(scenario, cohort, run_index)
                result = command_runner(_calibration_command(qwen_bin, worktree, prompt, qwen_model=qwen_model, qwen_fallback_model=qwen_fallback_model, model_selection=model_selection))
                live_calls_started = True
                transcript = _write_invocation_diagnostic(diagnostic_dir, f"{scenario}-{cohort}-{run_index:02d}", invocation_id, result)
                checks.append({"name": f"qwen-code-calibration-{scenario}-{cohort}-{run_index:02d}", "status": "PASS" if result.returncode == 0 else "FAIL", "details": {"returncode": result.returncode, "diagnostic": _display_path(transcript)}})
    ...
    if not blockers:
        ...
        _write_json(
            receipt_path,
            {
                ...
                "status": "PASS",
                "receiptId": "qwen-code-live-calibration",
                "host": HOST,
                ...
                "usageAttestationPolicy": budget_policy.usage_attestation_policy("qwen-code-json"),
...
def _usage_or_block(result: CommandResult, budget_policy: BudgetPolicy, blockers: list[dict[str, Any]], label: str) -> QwenCodeUsage | None:
    if result.returncode != 0:
        blockers.append({"code": "qwen-code-live-invocation-failed", "message": f"{label} returned {result.returncode}"})
        return None
...
def _operation_command(
    ...
    *,
    ...
    command.extend(_model_args(qwen_model, qwen_fallback_model, model_selection))
...
def _calibration_command(
    ...
    *,
    ...
    _ = worktree
    return [qwen_bin, "--safe-mode", "--output-format", "stream-json", *_model_args(qwen_model, qwen_fallback_model, model_selection), "--prompt", prompt]


def _model_args(
    qwen_model: str | None,
    qwen_fallback_model: str | None,
    model_selection: HostModelSelection | None,
) -> list[str]:
    model = qwen_model or (model_selection.provider_model if model_selection is not None else None)
    fallback_model = qwen_fallback_model or (model_selection.variant if model_selection is not None else None)
    args: list[str] = []
    if model:
