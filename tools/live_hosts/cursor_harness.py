...
class CursorUsage:
    ...
    def with_context_byte_proxy(self, value: int) -> "CursorUsage":
        return replace(
            self,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--diagnostic-dir", default="tasks/release-0-3/evidence/live-host-diagnostics/cursor")
...
def run_preflight(
    *,
    ...
    version = _run_command([cursor_bin, "agent", "--version"], checks, "cursor-agent-version")
    help_result = _run_command([cursor_bin, "agent", "--help"], checks, "cursor-agent-help")
    status = _run_command([cursor_bin, "agent", "status"], checks, "cursor-agent-status")
    about = _run_command([cursor_bin, "agent", "about"], checks, "cursor-agent-about")
    if cursor_model:
        models = _run_command([cursor_bin, "agent", "models"], checks, "cursor-agent-models")
        if models["returncode"] != 0 or cursor_model not in models["stdout"]:
...
def run_live_host_receipt(
    *,
    ...
    auth_checker = auth_checker or check_cursor_auth
    blockers: list[dict[str, Any]] = []
    checks: list[dict[str, Any]] = []
    baseline = _load_json(baseline_path)
    operations = required_operations(baseline)
    ...
    for index, operation_name in enumerate(operations, start=1):
        try:
            ...
            blockers.append({"code": error.code, "message": error.message})
            break
        invocation_id = f"cursor-{operation_name}-live-{index:02d}"
        prompt = _prompt_for_operation(operation_name)
        result = runner(_cursor_command(cursor_bin, prompt, worktree, mode=None, cursor_model=cursor_model), None)
    ...
    if not blockers:
        _write_json(
            receipt_path,
            {
                ...
                "status": "PASS",
                "receiptId": "cursor-live-host-conformance",
                "host": HOST,
                "adapterId": HOST,
    ...
    return _live_report("PASS" if not blockers else "FAIL", blockers, checks, budget_policy, budget_tracker, live_calls_started, len(operations), len(live_operations), receipt_path, cursor_model=cursor_model)
...
def run_live_calibration(
    *,
        ...
        for cohort in cohorts:
            for run_index in range(1, requested_runs + 1):
                try:
                    ...
                    blockers.append({"code": error.code, "message": error.message})
                    break
                invocation_id = f"cursor-{scenario}-{cohort}-{run_index:02d}"
                prompt = _prompt_for_calibration(scenario, cohort, run_index)
                result = runner(_cursor_command(cursor_bin, prompt, worktree, mode="plan", cursor_model=cursor_model), None)
                live_calls_started = True
    ...
    if not blockers:
        _write_json(
            receipt_path,
            {
                ...
                "status": "PASS",
                "receiptId": "cursor-live-calibration",
                "host": HOST,
                ...
                "usageAttestationPolicy": budget_policy.usage_attestation_policy("cursor-agent-stream-json"),
    ...
    report = _live_report("PASS" if not blockers else "FAIL", blockers, checks, budget_policy, budget_tracker, live_calls_started, required_invocations, len(runs), receipt_path, cursor_model=cursor_model)
...
def build_live_operation_record(*, host: str, name: str, invocation_id: str, usage: CursorUsage, output_identity: dict[str, Any]) -> dict[str, Any]:
...
def parse_cursor_jsonl(text: str, *, wall_seconds: float = 0.0) -> CursorUsage:
    ...
    tool_calls = 0
    for event in events:
        session_id = session_id or _find_string(event, {"session_id", "sessionId", "chat_id", "chatId"})
        usage_values.extend(_find_dicts(event, {"usage", "tokenUsage", "tokens"}))
        costs.extend(_find_numbers(event, {"cost_usd", "costUsd", "total_cost_usd", "totalCostUsd", "totalCostUSD"}))
        tool_calls += _count_tool_calls(event)
    input_tokens = sum(_int_from_any(_first_present(usage, ("inputTokens", "input_tokens", "prompt_tokens", "promptTokens", "input"))) for usage in usage_values)
    output_tokens = sum(_int_from_any(_first_present(usage, ("outputTokens", "output_tokens", "completion_tokens", "completionTokens", "output"))) for usage in usage_values)
    cache_tokens = sum(_int_from_any(_first_present(usage, ("cache_creation_input_tokens", "cacheCreationInputTokens", "cache_read_input_tokens", "cacheReadInputTokens", "cachedTokens", "cacheTokens"))) for usage in usage_values)
    total_tokens = sum(_int_from_any(_first_present(usage, ("billableTokens", "billable_tokens", "total_tokens", "totalTokens", "total"))) for usage in usage_values)
    context_bytes = sum(_int_from_any(_first_present(usage, ("cumulativeContextBytes", "cumulative_context_bytes", "contextBytes", "context_bytes"))) for usage in usage_values)
    billable_tokens = total_tokens or input_tokens + output_tokens + cache_tokens
    return CursorUsage(
...
def check_cursor_auth(cursor_bin: str) -> dict[str, Any]:
    status = run_command_capture([cursor_bin, "agent", "status"], None)
    about = run_command_capture([cursor_bin, "agent", "about"], None)
    return {
        "authenticated": _cursor_auth_ok({"returncode": status.returncode, "stdout": status.stdout, "stderr": status.stderr}, {"returncode": about.returncode, "stdout": about.stdout, "stderr": about.stderr}),
        "statusReturncode": status.returncode,
        "aboutReturncode": about.returncode,
        "statusStdoutSha256": sha256_hex(status.stdout.encode("utf-8")),
        "statusStderrSha256": sha256_hex(status.stderr.encode("utf-8")),
        "aboutStdoutSha256": sha256_hex(about.stdout.encode("utf-8")),
        "aboutStderrSha256": sha256_hex(about.stderr.encode("utf-8")),
    }
...
def _validate_calibration_inputs(profile: dict[str, Any], targets: dict[str, Any], blockers: list[dict[str, Any]], scenarios: list[str], cohorts: list[str], requested_runs: int, minimum_runs: int) -> None:
    ...
    if HOST not in _strings(profile.get("requiredHosts")):
        blockers.append({"code": "live-calibration-host-unsupported", "message": "cursor is not in requiredHosts"})
...
def _append_auth_check(blockers: list[dict[str, Any]], checks: list[dict[str, Any]], auth: dict[str, Any]) -> None:
    checks.append({"name": "cursor-agent-auth", "status": "PASS" if auth.get("authenticated") else "FAIL", "details": auth})
...
def _calibration_usage_or_block(result: CommandResult, budget_policy: BudgetPolicy, blockers: list[dict[str, Any]], label: str, prompt: str) -> CursorUsage | None:
...
def _cursor_command(cursor_bin: str, prompt: str, worktree: Path, mode: str | None, cursor_model: str | None = None) -> list[str]:
    command = [cursor_bin, "agent", "--print", "--output-format", "stream-json", "--workspace", str(worktree), "--trust"]
    if cursor_model:
        command.extend(["--model", cursor_model])
    if mode:
        command.extend(["--mode", mode])
    else:
        command.extend(["--force", "--sandbox", "enabled"])
...
def _prompt_for_calibration(scenario: str, cohort: str, run_index: int) -> str:
    return (
        "Agent Lifecycle Kit Cursor live calibration probe. "
...
def _cursor_auth_ok(status: dict[str, Any], about: dict[str, Any]) -> bool:
    combined = f"{status.get('stdout', '')}\n{status.get('stderr', '')}\n{about.get('stdout', '')}\n{about.get('stderr', '')}".lower()
    if "not logged in" in combined or "authentication required" in combined:
        return False
    if "user email" in combined and "not logged in" in combined:
        return False
    return status.get("returncode") == 0 and "not logged in" not in combined
...
def _sanitize_lines(text: str) -> list[str]:
    ...
    for line in text.splitlines():
        ...
        if "user email" in lower:
            lines.append("User Email <redacted-or-not-logged-in>")
        else:
