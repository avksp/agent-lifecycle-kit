...
def run_preflight(approval_path: Path) -> dict[str, Any]:
    ...
    receipt = None
    if not blockers:
        with tempfile.TemporaryDirectory(prefix="alk-claude-preflight-") as tmp:
            root = Path(tmp)
            profile_path = root / ".alk/host-launch/claude.json"
...
def run_planning_preflight(approval_path: Path) -> dict[str, Any]:
    ...
    if not blockers:
        with tempfile.TemporaryDirectory(prefix="alk-claude-planning-") as tmp:
            root = Path(tmp)
