...
WS-01 does not require this external authority to start the standalone
repository. Its bootstrap validation uses local controller receipts, stdlib
test runners and generic repository-owned neutrality policy. This avoids
committing origin-specific deny values into the standalone kit. The authority
below is mandatory for downstream pre-launch/post-attempt gates, release
neutrality and finalization neutrality, where host-owned origin-specific deny
values and signing material are required.
