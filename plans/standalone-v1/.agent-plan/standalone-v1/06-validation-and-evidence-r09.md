...
- WS-01 runs the complete negative suite and a controller-managed local
  bootstrap scan using only Python stdlib and repository-owned generic
  neutrality policy. WS-02 cannot launch until the local bootstrap evidence,
  negative-suite report and foundation report are accepted by controller
  validation receipts bound to the same source revision and plan digest.
- Origin-specific deny values are never committed into the standalone
  repository. They are supplied only to downstream runtime/release neutrality
  gates through the external `NEUTRALITY_*` authority contract.
...
- `neutrality-authority-profile.v3.json` is the exact machine authority for
  downstream gate identity, the `NEUTRALITY_*` external input boundary,
  freshness, signatures, write-once subject projection, archive limits and
  zero counters. Every downstream operation freezes an ordered exact-path
  output descriptor set. Each path must be unique and absent at scan and
  immediately before publication; primary artifacts are content-addressed and
  published create-if-absent before the detached signed receipt, which is
  published last as the commit marker. A committed same-operation replay is
  read-only and byte-identical; partial, occupied, reused, aliased,
  substituted or raced outputs fail closed and remain visible to later scans.
...
- Every schema 3 command has a positive timeout, one owner/task id and one
  bounded controller primary artifact. Commands that emit inspectable reports
  must declare those reports as controller protocol outputs and bind their
  exact frozen path in argv.
