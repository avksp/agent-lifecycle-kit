...
- AC-NEUTRALITY: WS-01 bootstrap emits fresh controller-managed local evidence:
  the generic repository-policy scan, the complete neutrality negative suite
  and the foundation suite all pass without `pytest`, network or host-operation
  approval. Every local report is a declared protocol output bound to the
  signed controller operation request, and every required result counter is
  zero. Origin-specific deny values remain outside the repository and are
  enforced by downstream/release `NEUTRALITY_*` gates.
