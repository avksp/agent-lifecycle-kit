...
- `git-worktree-v1` is the first change-set provider, behind a versioned
  provider interface.
- WS-01 bootstrap validation is locally reproducible with Python stdlib:
  controller-managed package tests, negative neutrality tests and a generic
  repository-policy scan must pass before downstream tasks can launch.
- External origin-specific deny values and Ed25519 trust roots are injected at
  runtime for downstream and release neutrality gates; they are never
  committed, logged or emitted in evidence. Conformance keys exist only inside
  temporary directories; production signing keys remain host-owned.
...
| Risk | Control | Owner |
| --- | --- | --- |
| Origin-specific information leaks into code, history, samples or evals | WS-01 generic local bootstrap scan plus downstream, per-attempt and full-release signed neutrality gates with external origin-specific deny authority | WS-01, WS-05, WS-14, WS-15 |
...
| Repository-controlled or stale external authority is accepted | Local bootstrap does not require origin-specific authority; downstream and release gates use one bounded fresh signed `NEUTRALITY_*` contract outside workspace, Git, artifact and release roots | WS-05, WS-14 |
...
Specification revisions 6 through 8 are retained as frozen lineage. Revision 9
refines the bootstrap neutrality split: WS-01 becomes autonomous on Python
stdlib and controller receipts, while host-owned origin-specific signed
neutrality remains mandatory for downstream and release gates.

Remaining sequence for revision 9: independent specification review and
freeze, deterministic plan validation, independent full plan review, then
fail-closed plan refreeze. No implementation task may launch or resume against
revision 9 before the new `plan.lock.json` and task packets are generated and
execution is explicitly reauthorized.
