...
security contract, fail-closed scanner, redacted attestation format and
complete negative suite. Execute and sign the full repository bootstrap scan.
Implement neutrality v3: exact operation-bound output descriptors, absent
write-once paths, content-bound primary artifacts, detached receipt published
last, portable no-follow stable reads, numeric archive limits, one bounded
fresh external `NEUTRALITY_*` authority contract and producer/verifier parity.
Occupied, partial, replay-mismatched, aliased, raced or archive-limit subjects
fail closed. This is the only first-wave implementation task; WS-02 cannot
launch until its fresh receipt verifies.
